...
from Rollar.tools import isint, closest_int_from_list, is_jsonable
...
class Logger():
    ...
    def __init__(self, settings):
        ...
        logging_config = {
            ...
            'loggers': {
                # all purpose, Rollar root logger
                'Rollar': {
            ...
            'handlers': {
                ...
                'logfile': {
                    ...
                    'filename': os.path.join(
                        self.config.internal["paths"]["logs_dir"], '.Rollar.log'),
                    'when': 'midnight',
                    'formatter': 'Rollar',
                },
                # Memory Logging
        ...
        self.logger = logging.getLogger('Rollar')
    ...
    def levelno(self):
        ...
        if isint(self.config.dict["logging"]["level"]):
            ...
            if self.config.dict["logging"]["level"].upper() not in levels:
                level = self.plex_themes.config.conf_default["logging"]["level"]["value"]
