...
import Plex_Themes.exceptions
from Plex_Themes.tools import isint, isfloat, is_arithmetic
...
class Config():
    ...
    def secondary_setup(self):
        """
        Perform setup post logger setup.
        """
    ...
    def core_setup(self, script_dir):
        ...
        Plex_Themes_web_dir = pathlib.Path(script_dir).joinpath('Plex_Themes_web')
        www_dir = pathlib.Path(Plex_Themes_web_dir).joinpath('www_dir')
        ...
        self.internal["paths"] = {
                                    ...
                                    "Plex_Themes_web_dir": Plex_Themes_web_dir,
                                    "www_dir": www_dir,
                                    "www_templates_dir": pathlib.Path(Plex_Themes_web_dir).joinpath('templates'),
        ...
        # Web Server Conf
        self.read_json_config(pathlib.Path(self.internal["paths"]["Plex_Themes_web_dir"]).joinpath("web_ui_conf.json"))
    ...
    def config_verification(self):
        ...
        if self.dict["main"]["cache_dir"]:
            ...
            if not pathlib.Path(self.dict["main"]["cache_dir"]).is_dir():
                raise Plex_Themes.exceptions.ConfigurationError("Invalid Cache Directory. Exiting...")
        ...
        self.dict["database"]["path"] = pathlib.Path(cache_dir).joinpath('plex_themes.db')
